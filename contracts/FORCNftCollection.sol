...
error FORCNftCollection__InvalidMintAmount();
error FORCNftCollection__MaxSupplyExceeded();
error FORCNftCollection__InsufficientFunds();
error FORCNftCollection__AllowlistSaleClosed();
error FORCNftCollection__AddressAlreadyClaimed();
error FORCNftCollection__InvalidProof();
error FORCNftCollection__PublicSaleClosed();
error FORCNftCollection__TransferFailed();
error FORCNftCollection__NonexistentToken();
...
contract FORCNftCollection is ERC721AQueryable, Ownable, ReentrancyGuard {
    ...
    modifier mintCompliance(uint256 _mintAmount, uint256 _maxMintAmount) {
        if (_mintAmount < 1 || _mintAmount > _maxMintAmount)
            revert FORCNftCollection__InvalidMintAmount();

        if (totalSupply() + _mintAmount > iMaxSupply)
            revert FORCNftCollection__MaxSupplyExceeded();
    ...
    modifier mintPriceCompliance(uint256 _mintAmount) {
        if (msg.value < sMintPrice * _mintAmount)
            revert FORCNftCollection__InsufficientFunds();
        ...
        mintPriceCompliance(_mintAmount)
    {
        if (sSaleState != SaleState.AllowlistOnly)
            revert FORCNftCollection__AllowlistSaleClosed();

        if (sAllowlistClaimed[_msgSender()])
            revert FORCNftCollection__AddressAlreadyClaimed();

        bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));

        if (!MerkleProof.verify(_merkleProof, sMerkleRoot, leaf))
            revert FORCNftCollection__InvalidProof();
        ...
        mintPriceCompliance(_mintAmount)
    {
        if (sSaleState != SaleState.PublicOpen)
            revert FORCNftCollection__PublicSaleClosed();
    ...
    function withdraw() public onlyOwner nonReentrant {
        ...
        if (!success) revert FORCNftCollection__TransferFailed();
    }
    ...
    {
        if (!_exists(_tokenId)) revert FORCNftCollection__NonexistentToken();
